...
export class Curios {
  ...
  constructor(kit: Kit, parent: pc.Entity, moon: pc.Entity) {
    ...
    // A cuckoo clock nailed to the stage-left wing.
    const clockAt = at("cuckoo");
    const clock = kit.group("cuckoo-clock", this.root, V(clockAt.x - 0.1, clockAt.y, clockAt.z));
    clock.setLocalEulerAngles(0, 90, 0);
    const wood = kit.material("oak-dark", palette.oakDark, 0.16);
    kit.primitive("case", "box", clock, V(), { x: 0.8, y: 0.9, z: 0.3 }, wood);
    kit.primitive("roof", "cone", clock, V(0, 0.62, 0), { x: 1.1, y: 0.4, z: 0.4 }, kit.material("oak", palette.oak, 0.18));
    kit.primitive("face", "cylinder", clock, V(0, 0.05, 0.16), { x: 0.5, y: 0.03, z: 0.5 }, cream, V(90, 0, 0));
    kit.primitive("hand", "box", clock, V(0, 0.12, 0.19), { x: 0.03, y: 0.18, z: 0.01 }, ink, pc.Vec3.ZERO, false);
    kit.primitive("hand", "box", clock, V(0.06, 0.05, 0.19), { x: 0.14, y: 0.03, z: 0.01 }, ink, pc.Vec3.ZERO, false);
    this.clockDoor = kit.primitive("door", "box", clock, V(0, 0.36, 0.16), { x: 0.2, y: 0.16, z: 0.02 }, kit.material("oak", palette.oak, 0.18), pc.Vec3.ZERO, false);
    this.cuckoo = kit.group("cuckoo", clock, V(0, 0.36, 0.05));
    kit.primitive("bird", "sphere", this.cuckoo, V(), { x: 0.16, y: 0.14, z: 0.2 }, kit.material("cuckoo-blue", new pc.Color(0.25, 0.45, 0.75), 0.3), pc.Vec3.ZERO, false);
    kit.primitive("beak", "cone", this.cuckoo, V(0, 0, 0.12), { x: 0.05, y: 0.1, z: 0.05 }, kit.material("nose", palette.gold, 0.28), V(90, 0, 0), false);
    for (const side of [-1, 1]) kit.primitive("weight", "cylinder", clock, V(side * 0.18, -0.9, 0.05), { x: 0.08, y: 0.25, z: 0.08 }, kit.material("gold", palette.gold, 0.72, 0.55), pc.Vec3.ZERO, false);
  ...
  update(now: number): void {
    ...
    this.cuckoo.setLocalPosition(0, 0.36, 0.05 + out * 0.32);
